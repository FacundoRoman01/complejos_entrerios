  ...
  return (
    <div
      style={{
        ...
        borderRadius: 16,
        overflow: 'hidden',
        boxShadow: '0 24px 50px -28px rgba(38,51,29,0.35)',
        display: 'flex',
    ...
    >
      <div 
        ...
        style={{ position: 'relative', aspectRatio: '16/11', overflow: 'hidden', background: '#e3ddcb', cursor: 'pointer' }}
      >
      ...
      <div style={{ padding: 'clamp(24px,2.6vw,34px)', display: 'flex', flexDirection: 'column', flex: 1 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#a08b52' }}>
          {cabin.eyebrow}
        </div>
        
        <div 
          onClick={onOpenDetail}
          style={{ fontFamily: "'Raleway',sans-serif", fontSize: 'clamp(34px,3.4vw,46px)', lineHeight: 1, color: '#3a4a28', marginTop: 8, cursor: 'pointer' }}
        >
          {cabin.name}
        </div>
        
        <div style={{ fontSize: 13, letterSpacing: '0.06em', color: '#7c8a4e', marginTop: 14 }}>
          {cabin.specs}
        </div>
        
        <div style={{ height: 1, background: 'rgba(38,51,29,0.12)', margin: '22px 0' }} />
        
        <p style={{ fontSize: 14, lineHeight: 1.75, color: '#5b6349', margin: 0, fontWeight: 300 }}>
          {cabin.desc}
        </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginTop: 'auto' }}>
          <button
            // Aquí conectamos la función para que al apretar el botón se abra el modal
            onClick={onOpenDetail}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); openWhatsApp(waMessages.loma)(); }}
